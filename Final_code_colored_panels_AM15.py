...
def integrate_visual_spectrum(wavelengths, reflected, total, x_bar, y_bar, z_bar):
    ...
    X = refl_x/(total_spectrum_y)
    Y = refl_y/(total_spectrum_y)
    Z = refl_z/(total_spectrum_y)
...
def plot_spectra_comparison(Font,wavelengths, ref_fit, solar_rad, transmitted, reflected, filename):
    ...
    ax1.set_ylabel('Intensity [W/(m²·nm)]', fontsize=Font)
...
def plot_current_comparison(Font,wavelengths, current_color, filename, current_black, rgblist):
    ...
    plt.ylabel(r'$J_{SC}$ [A/(m²·nm)]', fontsize=Font)
    plt.grid(True)
    plt.savefig(filename, dpi=400)
...
def plot_transmission_comparison(Font,wavelengths, trans_orig, trans_shift, shift,image_path):
    ...
    plt.yticks(fontsize=Font*0.7)
    plt.xlabel('Wavelength [nm]', fontsize=Font)
    plt.ylabel('Intensity [W/(m²·nm)]', fontsize=Font)
...
def plot_qe_comparison(Font,qe_y, wavelengths, transmitted, current_color, image_path):
    ...
    ax1.tick_params(axis='y', labelsize=Font*0.7)
    ax1.set_xlabel('Wavelength [nm]', fontsize=Font)
    ax1.set_ylabel('Current [A/(m²·nm)]', fontsize=Font)
...
def load_data():
    ...
    data = {
        ...
        'solar_spectrum': np.loadtxt('am1.5_spectrum.txt', delimiter='\t',skiprows=1),
...
def process_data(data, amount_of_points_skipped,shift,points_skipped):
    """interpolate reflection, transmission and quantum efficiency data"""
    processed = {}
    
    # Wavelengths array from data
    wavelengths = data['solar_spectrum'][amount_of_points_skipped:points_skipped, 0]
    ...
    # Shift reflection
    ref_shift = np.roll(processed['ref_y'], shift)
...
def calculate_spectra(amount_of_points_skipped,processed, data_solar_spectrum, points_skipped):
    """Calculate various spectra based on processed data"""
    spectra = {}
    
    # solar spectrum from data
    solar_spectrum=data_solar_spectrum[amount_of_points_skipped:points_skipped,1]
...
def main():
    ...
    amount_of_points_skipped=140
    points_capped=991
    
    # (i+1) th element of array at which the visual spectrum starts and ends
    # Start calculating after the amount of points skipped!
    start_visual=25
    end_visual=401
    image_path = 'Plots_AM1.5'
    ...
    # Calculate wavelength diff per datapoint, assumes this remains steady
    del_wavelength=data['solar_spectrum'][300,0]-data['solar_spectrum'][299,0]

    # At which wavelength shift do we want to make the plots?
    shift_plot=int(100/del_wavelength)
    ...
    with open(output_filename, 'w') as f:
        ...
        for shift in shift_list:
            ...
            processed = process_data(data, amount_of_points_skipped, shift, points_capped)
            
            # Calculate reflected, absorbed, transmitted, shifted spectra
            spectra = calculate_spectra(amount_of_points_skipped,processed, data['solar_spectrum'],points_capped)
    ...
    if (plot==1):
        # Process data, interpolate:
        processed = process_data(data, amount_of_points_skipped, shift_plot, points_capped)
            
        # Calculate reflected, absorbed, transmitted, shifted spectra
        spectra = calculate_spectra(amount_of_points_skipped,processed, data['solar_spectrum'], points_capped)

        # Calculate color visualization for spectra and shifted spectra
        rgb_list= process_spectrum_color(processed['wavelengths'], spectra['reflected'], spectra['solar'], start_visual,end_visual)
        rgb_list_shift = process_spectrum_color(processed['wavelengths'], spectra['reflected_shift'], spectra['solar'], start_visual,end_visual)
        ...
        plot_spectra_comparison(Font,processed['wavelengths'], processed['ref_shift_y'],
                            ...
                            image_path+f'/Absorption_and_Radiation_Spectra_shifted_{int(del_wavelength*shift_plot)}.png')
        ...
        plot_current_comparison(Font,processed['wavelengths'], spectra['current_color_shift'],
                            image_path+f'/Absorption_and_Radiation_Current_shifted_{int(del_wavelength*shift_plot)}.png',spectra['current_black']
                            ,rgb_list_shift)
        ...
        plot_transmission_comparison(Font,processed['wavelengths'],
                                ...
                                int(del_wavelength*shift_plot), image_path)
